...
local function uiActivatedCallback(e)
	...
	if ADD_BUTTONS then
		...
		local button_block = bookMenu:createRect({ id = "rf_bookmenu_read", color = { 0, 0, 0 } })
		button_block.autoHeight = true
		button_block.autoWidth = true
		button_block.alpha = 1.0
		button_block.absolutePosAlignX = 0.5
		button_block.absolutePosAlignY = 0.97

		local bg = button_block:createThinBorder{}
		bg.autoHeight = true
		bg.autoWidth = true
		bg.paddingAllSides = 5
		bg.widthProportional = 1.0 -- width is 100% parent width

		local button_read = bg:createButton{ id = "rf_bookmenu_btn_read", text = "Read" }
		local button_stop = bg:createButton{ id = "rf_bookmenu_btn_stop", text = "Stop" }

		button_read:register(tes3.uiEvent.mouseClick, onMenuBookRead)
		button_stop:register(tes3.uiEvent.mouseClick, onMenuBookStop)

		button_block:updateLayout()
