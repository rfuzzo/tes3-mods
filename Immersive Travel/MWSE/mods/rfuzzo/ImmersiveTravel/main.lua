...
local sn = require("rfuzzo.ImmersiveTravel.data.sn-bm")
...
local function onTestOK(e)
	...
	if (menu) then
		myTimer = timer.start({ duration = timertick, type = timer.real, iterations = -1, callback = onTimerTick })
...
local function updateMarkers()
	...
	for index, marker in ipairs(editor_markers) do
		if index < #editor_markers then
			...
			-- dist:normalize()
			local rotz = tes3vector3.new(0, 1, 0):angle(tes3vector3.new(dist.x, dist.y, 0))
...
---@return number
local function getClosestMarker()
	-- get closest marker
...
local function onLogCommand(e)
	-- createLogWindow()
	mwse.log(tes3.player.position.x .. "," .. tes3.player.position.y .. "," .. tes3.player.position.z)
	tes3.messageBox(tes3.player.position.x .. "," .. tes3.player.position.y .. "," .. tes3.player.position.z)
...
event.register("simulate", function(e)
	...
	local pos = tes3.getPlayerEyePosition() + tes3.getPlayerEyeVector() * 900
	editor_instance.translation = pos
...
local function keyDownCallback(e)
	...
	if e.keyCode == tes3.scanCode["keyLeft"] then
		...
		local v = tes3.getPlayerEyePosition() + tes3.getPlayerEyeVector() * 256
		child.translation = tes3vector3.new(v.x, v.y, v.z)
	...
	if e.keyCode == tes3.scanCode["keyRight"] then
		...
		for _, value in ipairs(editor_markers) do
			...
			mwse.log("{ x = " .. math.round(p.x) .. ", y = " .. math.round(p.y) .. ", z = " .. math.round(p.z) .. " },")
		end
		mwse.log("============================================")
	end
end
